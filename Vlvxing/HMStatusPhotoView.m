...
- (void)setPhoto:(HMPhoto *)photo
{
    ...
    // 1.下载图片
    [self sd_setImageWithURL:[NSURL URLWithString:photo.url] placeholderImage:[UIImage imageWithName:@"timeline_image_placeholder"]];
    
    // 2.控制gif图标的显示
    if (photo.url) {
        NSString *extension = photo.url.pathExtension.lowercaseString;
