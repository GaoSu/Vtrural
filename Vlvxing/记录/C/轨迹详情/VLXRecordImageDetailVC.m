...
@interface VLXRecordImageDetailVC ()<ShareBtnViewDelegate>
...
@implementation VLXRecordImageDetailVC
...
-(void)createUI
{
    ...
    NSLog(@"%@",_model.picurl);
    [_iconImageView sd_setImageWithURL:[NSURL URLWithString:[ZYYCustomTool checkNullWithNSString:_model.picurl]] placeholderImage:NoDataImage_V];
    //
...
-(void)createUIWithCourse
{
    ...
    _iconImageView=[[UIImageView alloc] initWithFrame:CGRectMake(0, 0, kScreenWidth, kScreenHeight-64-80)];
    [self.view addSubview:_iconImageView];
    NSLog(@"%@",_courseModel.picUrl);
    [_iconImageView sd_setImageWithURL:[NSURL URLWithString:[ZYYCustomTool checkNullWithNSString:_courseModel.picUrl]] placeholderImage:NoDataImage_V];
    //
    _bottomView=[[UIView alloc] initWithFrame:CGRectMake(0, CGRectGetMaxY(_iconImageView.frame), kScreenWidth, 80)];
    _bottomView.backgroundColor=[UIColor whiteColor];
    [self.view addSubview:_bottomView];
...
-(void)createUIWithDetail
{
    ...
    _iconImageView=[[UIImageView alloc] initWithFrame:CGRectMake(0, 0, kScreenWidth, kScreenHeight-64-80)];
    [self.view addSubview:_iconImageView];
    NSLog(@"%@",_detailModel.picurl);
    [_iconImageView sd_setImageWithURL:[NSURL URLWithString:[ZYYCustomTool checkNullWithNSString:_detailModel.picurl]] placeholderImage:NoDataImage_V];
    //
    _bottomView=[[UIView alloc] initWithFrame:CGRectMake(0, CGRectGetMaxY(_iconImageView.frame), kScreenWidth, 80)];
    _bottomView.backgroundColor=[UIColor whiteColor];
    [self.view addSubview:_bottomView];
...
-(void)thirdShareWithUrl:(NSString * )url
{
    ...
    __block VLXRecordImageDetailVC * blockSelf=self;;
    ...
    if (_isShowMenu) {
        ...
        _menuView.menuBlock=^(NSInteger index)
        {
            ...
            if (index==100) {
                NSLog(@"分享");
                if (blockSelf.model) {
                    [blockSelf thirdShareWithUrl:[NSString stringWithFormat:@"%@/shareurl/zhaopianshare.json?pathInfoId=%@",ftpPath,blockSelf.model.pathinfoid]];
                }else if (blockSelf.courseModel)
                {
                    
                }else if (blockSelf.detailModel)
                {
                    [blockSelf thirdShareWithUrl:[NSString stringWithFormat:@"%@/shareurl/zhaopianshare.json?pathInfoId=%@",ftpPath,blockSelf.detailModel.pathinfoid]];
                }

            }else if (index==101)
            {
                NSLog(@"编辑");
                VLXRecordEditImageVC *editVC=[[VLXRecordEditImageVC alloc] init];
                if (blockSelf.model) {
                    editVC.model=blockSelf.model;
                }else if (blockSelf.courseModel)
                {
                    
                }else if (blockSelf.detailModel)
                {
                    editVC.detailModel=blockSelf.detailModel;
                }
                [blockSelf.navigationController pushViewController:editVC animated:YES];
            }else if (index==102)
            {
                NSLog(@"删除");
