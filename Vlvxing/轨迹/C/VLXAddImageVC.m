...
@interface VLXAddImageVC ()<UITextFieldDelegate,BMKGeoCodeSearchDelegate>
...
{
    if (_location==nil) {
        [SVProgressHUD showInfoWithStatus:@"定位获取失败，请重试"];
        return;
    }
    NSLog(@"点击保存图片按钮");
    if (_type==1) {//1表示单个点
...
//            NSLog(@"上传返回OK%@",url);
            ...
            [SPHttpWithYYCache postRequestUrlStr:saveUrl withDic:dic success:^(NSDictionary *requestDic, NSString *msg) {
                ...
                if ([requestDic[@"status"] integerValue]==1) {
                    ...
                    model.picUrl=[ZYYCustomTool checkNullWithNSString:url];
                    model.videoUrl=@"";
                    ...
                    NSMutableArray *eventsArray=[NSMutableArray arrayWithArray:[defaults objectForKey:@"courseSingleEvents"]];//表示单个事件数组
    ...
    }else if (_type==2)//2表示轨迹
    {
        ...
        [UploadImageTool UploadImage:_image upLoadProgress:^(float progress) {
            ...
            //保存到本地
            NSUserDefaults *defaults=[NSUserDefaults standardUserDefaults];
            NSMutableArray *eventsArray=[NSMutableArray arrayWithArray:[defaults objectForKey:@"courseEvents"]];//表示轨迹事件数组
            NSData *data=[NSKeyedArchiver archivedDataWithRootObject:model];
            [eventsArray addObject:data];
