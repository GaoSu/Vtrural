...
//
//  Created by 谭真 on .
//  Copyright ©  谭真. All rights reserved.
//
...
- (void)configBottomToolBar {
    TZImagePickerController *tzImagePickerVc = (TZImagePickerController *)self.navigationController;
    if (tzImagePickerVc.maxImagesCount < 1) return;//不能等于,不然后边不走
...
//    NSLog(@"bottomToolBarframe:%@",NSStringFromCGRect(bottomToolBar.frame));
...
- (void)refreshBottomToolBarStatus {
    ...
    TZImagePickerController *tzImagePickerVc = (TZImagePickerController *)self.navigationController;
    //****************弹出提示框操作的地方*******************
    NSMutableArray * seleMediaData = [[NSMutableArray alloc]init];
    for (TZAssetModel * model in tzImagePickerVc.selectedModels) {
        NSLog(@"选取的文件类型==%lu",(unsigned long)model.type);
        //思路:遍历数组,将数据源分成两类,一类是0,一类是2,然后判断2的个数,如果大于等于2,就弹出提示框
        
        
        if(((unsigned long)model.type == 2) && (![[[NSUserDefaults standardUserDefaults]objectForKey:@"CanAdd"] isEqualToString:@"NO"]))
        {
            NSLog(@"选择的是视频,并且当前是可以添加的,那么添加,并且设置BOOL为不能添加");
            [seleMediaData addObject:model];
            [[NSUserDefaults standardUserDefaults]setObject:@"NO" forKey:@"CanAdd"];
            
            if (seleMediaData.count ==2) {
                [tzImagePickerVc.selectedModels removeLastObject];
                UIAlertView *alt = [[UIAlertView alloc]initWithTitle:@"提示" message:@"只能选一个视频" delegate:self cancelButtonTitle:@"确定" otherButtonTitles:nil, nil];
                [alt show];
            }
            
        }else if(((unsigned long)model.type == 2) && [[[NSUserDefaults standardUserDefaults]objectForKey:@"CanAdd"] isEqualToString:@"NO"]){
            NSLog(@"@@@@@@@@@@@@@@@@@@@@@@%ld",tzImagePickerVc.selectedModels.count);
                [tzImagePickerVc.selectedModels removeLastObject];
            NSLog(@"_______________________%ld",tzImagePickerVc.selectedModels.count);

            UIAlertView *alt = [[UIAlertView alloc]initWithTitle:@"提示222222" message:@"只能选一个视频" delegate:self cancelButtonTitle:@"确定" otherButtonTitles:nil, nil];
            [alt show];
            break;
        }
