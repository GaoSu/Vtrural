...
//        self.navigationBar.barTintColor = [UIColor colorWithRed:(34/255.0) green:(34/255.0)  blue:(34/255.0) alpha:1.0];
    self.navigationItem.leftBarButtonItem.title = @"相册分类";
    self.navigationController.navigationItem.leftBarButtonItem.title = @"33333";
    self.navigationBar.tintColor = [UIColor blueColor];//蓝色箭头
...
- (instancetype)initWithMaxImagesCount:(NSInteger)maxImagesCount columnNumber:(NSInteger)columnNumber delegate:(id<TZImagePickerControllerDelegate>)delegate pushPhotoPickerVc:(BOOL)pushPhotoPickerVc {
    ...
    if (self) {
        self.maxImagesCount = maxImagesCount > 0 ? maxImagesCount : 9; // Default is 9 / 默认最大可选9张图片//2018.1.30修改为6张
        ...
        self.barItemTextColor = [UIColor blueColor];
...
- (instancetype)initWithSelectedAssets:(NSMutableArray *)selectedAssets selectedPhotos:(NSMutableArray *)selectedPhotos index:(NSInteger)index{
    ...
    if (self) {
        ...
        self.photoPreviewMaxWidth = 600;
        self.barItemTextFont = [UIFont systemFontOfSize:15];
        self.barItemTextColor = [UIColor orangeColor];
        [self configDefaultImageName];
...
- (void)configDefaultImageName {
    ...
    self.photoSelImageName = @"photo_sel_photoPickerVc.png";//选中的图或视频
    self.photoDefImageName = @"photo_def_photoPickerVc.png";//未选中的,
