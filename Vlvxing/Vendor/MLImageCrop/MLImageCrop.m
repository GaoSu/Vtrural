...
@property(nonatomic,assign) CGFloat recordedRotation;
@end
...
{
    [self disappear];
    if (self.delegate && [self.delegate respondsToSelector:@selector(corpImageDismiss)]){
        [self.delegate corpImageDismiss];
    }
}
...
- (void) rotateView:(UIRotationGestureRecognizer *)rotationGestureRecognizer
{
    ...
    if (rotationGestureRecognizer.state == UIGestureRecognizerStateBegan || rotationGestureRecognizer.state == UIGestureRecognizerStateChanged) {
        view.transform = CGAffineTransformRotate(view.transform, -(_recordedRotation - rotationGestureRecognizer.rotation));
        _recordedRotation = rotationGestureRecognizer.rotation;
        MyLog(@"%f",rotationGestureRecognizer.rotation);
    }
    else if(rotationGestureRecognizer.state == UIGestureRecognizerStateEnded){
        if (self.image.imageOrientation == UIImageOrientationUp) {            UIImage *img = [UIImage imageWithCGImage:self.image.CGImage scale:1 orientation:UIImageOrientationRight];
            _image = img;
            ...
            UIImage *img = [UIImage imageWithCGImage:self.image.CGImage scale:1 orientation:UIImageOrientationUp];
            _image = img;
            self.imageView.image = img;
            if (self.isViewLoaded) {
                [self.view setNeedsLayout];
