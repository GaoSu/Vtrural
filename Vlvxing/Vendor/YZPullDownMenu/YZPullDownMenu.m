...
//        CGFloat coverW = kScreenWidth;
        ...
        _coverView.clickCover = ^{ // 点击蒙版调用
            ...
            if (self.dataSource && [self.dataSource respondsToSelector:@selector(coverViewClick)]) {
                [weakSelf.dataSource coverViewClick];
            }
...
//        NSInteger col = [self.controllers indexOfObject:note.object];
        ...
        for (int i= 0 ; i<self.controllers.count; i++) {
            ...
            id  controller=self.controllers[i];
        ...
        // 获取所有值
//        NSArray *MyAllValues = note.userInfo.allValues;
//        NSString *count = [MyAllValues lastObject];
//        NSInteger col =[count integerValue];
        
        // 获取对应按钮
        if (self.menuButtons.count<col) {
            return ;
        }
...
- (void)btnClick:(UIButton *)button
{
    ...
    //判断是否要显示下面的cover
    if (self.dataSource && [self.dataSource respondsToSelector:@selector(pullDownMenu:heightForColAtIndex:)]) {
        
     CGFloat height =  [self.dataSource pullDownMenu:self heightForColAtIndex:button.tag];
    ...
    if (button.selected == YES) { // 当按钮选中，弹出蒙版
        ...
        if (self.dataSource && [self.dataSource respondsToSelector:@selector(pullDownMenu:heightForColAtIndex:)]) {
             ...
             height =  [self.dataSource pullDownMenu:self heightForColAtIndex:button.tag];
