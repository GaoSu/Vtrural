...
#define rightItemTitle [NSString stringWithFormat:@"完成 %ld/%ld",(long)isChoosenDic.count,(long)self.maxCount]
//#define rightItemTitle @"完成"
...
- (void)initDefaultView {
    ...
    [titleBtn setImage:[UIImage imageNamed:@"YBimgPickerView.bundle/jiantou"] forState:UIControlStateNormal];
    [titleBtn setImageEdgeInsets:UIEdgeInsetsMake(0, 75, 0, 0)];
...
//    [titleBtn.titleLabel setFont:[UIFont fontWithName:FONT_NAME size:20]];
    [titleBtn.titleLabel setFont:[UIFont systemFontOfSize:18]];
    ...
    myCollectionView.backgroundColor = [UIColor colorWithHexString:@"ececec"];
    //backView
    isShowTable = NO;
    backView.alpha = 0;
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath {
    if (indexPath.item == 0) {
        ...
        if (self.maxCount > 0) {
            if (isChoosenDic.count >= self.maxCount) {
                return;
            }
        }else if (self.onlyOne) {
            if (isChoosenDic.count > 1) {
                return;
            }
        }else {
...
//            }
        ...
        preViewController.maxCount = _maxCount;
...
//            [isChoosenDic setObject:[UIImage imageWithData:UIImageJPEGRepresentation(image, 0.5)] forKey:[asset description]];
        }
        UIImage * image = [UIImage imageWithCGImage:[[asset defaultRepresentation] fullScreenImage]];
        image = [self fixOrientation:image];
        [isChoosenDic setObject:[UIImage imageWithData:UIImageJPEGRepresentation(image, 0.5)] forKey:[asset description]];
...
- (void)imagePickerController:(UIImagePickerController *)picker didFinishPickingMediaWithInfo:(NSDictionary *)info {
    ...
    if (image) {
        ...
        if (picker.sourceType == UIImagePickerControllerSourceTypeCamera)
        {
            ...
            [isChoosenDic setObject:[UIImage imageWithData:UIImageJPEGRepresentation(newOriImg, 0.5)] forKey:@"Camera"];
        }
        __weak typeof(self) weakSelf = self;
